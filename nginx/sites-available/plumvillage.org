...
server {
	...
	add_header "max-age=86400, public, must-revalidate, proxy-revalidate";
...
server {
	listen [::]:8080;
	server_name www.plumvillage.org;
	rewrite ^ $scheme://plumvillage.org$request_uri? permanent;
}

# Redirect all new. queries to non-www
server {
	listen [::]:8080;
	server_name new.plumvillage.org;
	rewrite ^ $scheme://plumvillage.org$request_uri? permanent;
}
